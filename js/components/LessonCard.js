// ============================================================
// LessonCard — the main lesson display component. Renders an
// accepted lesson (or option preview) with all sections: header,
// context strip, at-a-glance, curriculum link, board layout,
// materials, phases (with scripts/responses), checkForUnderstanding,
// embeddedContent, exit ticket, differentiation, extras, pitfalls,
// classroom mgmt, teacher notes, homework, assessment, sub tip,
// mark-as-used button, sub feedback section.
...
// Section: little subheading with icon + label
...
window.LessonCard = function LessonCard({ lesson, t, onPick, isOption, optionLabel, onMarkUsed, onSubmitFeedback }) {
  ...
  // Rerender on library changes (feedback added).
  ...
  return (
    <div style={{
      ...
      {/* Context strip — teacher/class/day/time/room */}
      {!isOption && hasCtx && (
        <div style={{
      ...
      <div style={{ display: "grid", gap: 16 }}>
        {/* AT-A-GLANCE: objective / prior knowledge / criteria */}
        ...
        {/* Board layout — verbatim text to copy onto the board */}
        {lesson.boardLayout && (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 4 }}>
              {lesson.phases.map((phase, i) => (
                <div key={i} style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-md)", padding: "14px 16px" }}>
                  ...
                  {/* Teacher script — exact phrases the teacher can say */}
        ...
        {/* Legacy fallback for older lesson shapes */}
        ...
        {/* Embedded content (sub-lesson questions, problems, texts, example answers) */}
            ...
            <div style={{ display: "grid", gridTemplateColumns: compact ? "1fr" : "1fr 1fr", gap: 12 }}>
              ...
              {lesson.differentiation.utmaning && (
                <div style={{ background: "#FAF6FC", border: "1px solid #DBC9E5", borderRadius: "var(--radius-md)", padding: "10px 12px" }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "#6B3E8E", marginBottom: 5 }}>
                    ⚡ {t.differentiationUtmaning}
                  </div>
                  <ul style={{ fontSize: 13, lineHeight: 1.55, listStyle: "none", display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        {/* Sub tip — one-line takeaway for the substitute */}
        {lesson.subTip && (
      ...
      {/* Mark-as-used + Sub feedback — only on accepted (non-option) lesson */}
      ...
      {/* Option preview gets a "Choose this" button at the bottom */}
      {isOption && (
